...
# Obtiene la API Key de OpenAI desde los secrets de Streamlit Cloud (recomendado)
openai.api_key = st.secrets.get("OPENAI_API_KEY", "")

# ------------------------------------------------------------------
# DATOS DE CANDIDATOS (FOTOS OFICIALES / LICENCIAS ABIERTAS)
# Se utilizan imágenes de dominio público / licencias libres alojadas en Wikipedia Commons.
# En producción, reemplaza con fotos oficiales autorizadas por cada comando de campaña.
# ------------------------------------------------------------------
CANDIDATOS = {
...
# Fuente oficial: Servicio Electoral de Chile (Servel) vía BCN / Centro de Datos Servel.
# Gabriel Boric 55.87% (4,621,231 votos) | José Antonio Kast 44.14% (3,650,662 votos).
# ------------------------------------------------------------------
RESULTADOS_2021 = {
...
with tabs[0]:
    ...
    if st.button("Verificar con DATIA"):
        if not afirmacion.strip():
            ...
            st.error("Falta la API Key de OpenAI. Configúrala en Streamlit Cloud (Settings → Secrets).")
        else:
            with st.spinner("DATIA verificando fuentes oficiales..."):
                prompt = build_verifier_prompt(afirmacion)
                try:
                    # Nota: ChatCompletion legacy; puede migrarse a Responses API en versiones nuevas del SDK.
...
# Muestra datos oficiales de elecciones pasadas (2021) o estado de 2025.
# ------------------------------------------------------------------
with tabs[1]:
...
# Muestra foto + espacio para texto (editable en el futuro).
# ------------------------------------------------------------------
with tabs[2]:
